...
import play.mvc.*;
import util.Create;
import util.DatasetMemento;
import util.Retrieve;

@Named
@Singleton
public class DatasetController extends Controller implements Retrieve, Create, DatasetMemento {
	...
	private Hashtable<Long, Dataset> memento;
	...
	public Result add() {
    	...
    	try {
			...
			long Id = saveDatasetMemento(dataset);
			System.out.println("Dataset saved: "+ Id);
    ...
    public Result updateDatasetById(long id) {
		...
		try {
			Dataset saved = getDatasetMemento(id);
			...
			long Id = saveDatasetMemento(dataset);
			System.out.println("Dataset updated: "+ Id);
			return created("Dataset updated: "+ Id);
		} catch (PersistenceException pe) {
			pe.printStackTrace();
	...
	@Override
	public Result getAll(String format) {
		try {
			Iterable<Dataset>datasets =  datasetRepository.findAll();
			String result = new String();
			result = new Gson().toJson(datasets);
			System.out.println("Retrieve all dataset");
			return ok(result);
		} catch (Exception e) {
	...
	public long saveDatasetMemento(Dataset dataset) {
		Dataset saved = this.datasetRepository.save(dataset);
		long id =  saved.getId();
		this.memento.put(id, saved);
		System.out.println("save dataset memento: " + saved.toString());
		return id;
	...
	public Dataset getDatasetMemento(long id) {
		if (this.memento.containsKey(id)) {
			System.out.println("get dataset memento: " + this.memento.get(id));
	...
	public void deleteDatasetMemento(long id) {
		if (!this.memento.containsKey(id)) {
			...
			System.out.println("delete dataset memento: " + this.memento.get(id));
