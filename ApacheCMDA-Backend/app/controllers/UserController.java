...
public class UserController extends Controller {
	...
	public Result userLogin() {
		...
		if(email.trim().length() == 0) {
			...
			return ok(new Gson().toJson(factory.getLoginError("EmptyEmail").errorResponse()));
		}

		if(password.trim().length() == 0) {
			System.out.println("Input password is empty");
			return ok(new Gson().toJson(factory.getLoginError("EmptyPassword").errorResponse()));
		}
        ...
        // 2. User valid
        // 3. User not valid
        // the context behavior will changed based ont the type of the state.

        Context context = new Context();

		if(user == null) {
            UserNotExistState userNotExistState = new UserNotExistState();
            return ok(userNotExistState.doAction(context, null));
		}

		// if (user.getPassword().equals(MD5Hashing(password))) {
		if (user.getPassword().equals(password)) {
            UserValidState userValidState = new UserValidState();
            return ok(userValidState.doAction(context, user));
		} else {
            UserNotValidState userNotValidState = new UserNotValidState();
            return ok(userNotValidState.doAction(context, user));
		}
	}
